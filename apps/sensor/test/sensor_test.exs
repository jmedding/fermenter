defmodule SensorTest do
  ...
  test "Can start a sensor", %{type: type, gpio: gpio, sensor_mod: sensor_mod} do
    ...
    assert {:ok, pid, _struct} = Sensor.start_sensor(sensor_mod, type, gpio, name)
    Process.exit(pid, :shutdown)
    inspect Supervisor.count_children(SensorSupervisor)
    inspect Supervisor.count_children(SensorSupervisor)
    assert %Sensor.Temp{ value: value } = Server.read(name)
    assert value == 20.0
  ...
  test "Can start more than one sensor", %{type: type, gpio: gpio, sensor_mod: sensor_mod} do
    {name1, name2} = {:c, :d}
    #assert {:ok, _sup} = Sensor.Supervisor.start_link
    assert {:ok, _pid, _struct} = Sensor.start_sensor(sensor_mod, type, gpio, name1)
    assert {:ok, _pid, _struct} = Sensor.start_sensor(sensor_mod, type, gpio, name2)
    inspect Supervisor.count_children(SensorSupervisor)
    inspect Supervisor.count_children(SensorSupervisor)
    Server.set_temp(name1, 22)
    assert %Sensor.Temp{ value: value1 } = Server.read(name1)
    assert %Sensor.Temp{ value: value2 } = Server.read(name2)
    assert value1 == 22.0
    assert value2 == 20.0
  end
